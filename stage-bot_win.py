...
def do_screenshot(device):
    ...
    with open('screen.png', 'wb') as fd:
        fd.write(image)
    with Image.open('screen.png') as fd:
...
def check_rgb(device, location, rgb):
    image = do_screenshot(device)
    r,g,b = [int(j) for j in [list(i[:3]) for i in image[location['y']]][location['x']]]
    if {'r': r, 'g': g, 'b': b} == rgb:
        return True
    else:
        return False
...
if __name__ == "__main__":
    ...
    if not args.job_list and not args.only_automate and not args.show and not args.debug:
        parser.print_help()
        exit(1)
    elif args.show:
        date_now = datetime.now(timezone.utc).astimezone(timezone(timedelta(hours=-11))).strftime('%a')
